...
/*static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
	  // Define the string to compare
		    const char *led_string = "LED";

		    // Compare the received buffer with the LED string
		    if (strncmp((char*)Buf, led_string, strlen(led_string)) == 0)
		    {
		        // If the buffer matches the LED string, toggle the LED
...
/*
	  ...
	  //tranmitting ldr val to uart
	  //char msg[50];
	  sprintf(msg, "LDR Value: %lu\r\n", adc_value);
	  HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);

	  char ldr_vstr[10];
	  snprintf(ldr_vstr,sizeof(ldr_vstr),"%lu\n",value);
	  CDC_Transmit_FS((uint8_t*)ldr_vstr, strlen(ldr_vstr));
0
	  HAL_Delay(1000);
	  // Toggle LED based on ldr value
	  if (value > 1024)
	  {
		  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_SET); // Turn on LED
	  }
